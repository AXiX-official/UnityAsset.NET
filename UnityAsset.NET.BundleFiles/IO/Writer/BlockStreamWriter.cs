...
namespace UnityAsset.NET.IO.Writer
{
    ...
    public abstract class BlockStreamWriter : IWriter, IDisposable
    {
        ...
        public readonly List<StorageBlockInfo> BlockInfos = new List<StorageBlockInfo>();
        ...
        public abstract void WriteStream(Stream stream);
    ...
    public class NoneCompressionBlockStreamWriter : BlockStreamWriter
    {
        ...
        protected override void CheckMemoryLimit()
        {
            if (IsMemoryStream && Stream.Length >= InMemorySizeLimit)
            {
                ...
                var newStream = new FileStream(path, FileMode.Create, FileAccess.ReadWrite, FileShare.ReadWrite, 4096,
    ...
    public class Lz4CompressionBlockStreamWriter : BlockStreamWriter
    {
        ...
        public override void WriteStream(Stream stream)
        {
            ...
            {
                var bytesRead = stream.Read(_buffer, _bufferPos, BuffRemaining);
                _position += bytesRead;
                _bufferPos += bytesRead;
    ...
    public class LzmaCompressionBlockStreamWriter : BlockStreamWriter
    {
        ...
        public override void WriteStream(Stream stream)
        {
            var bufferLength = Math.Min(8196, stream.Length);
            var tempBuffer = new byte[bufferLength];
            while (stream.Position < stream.Length)
            {
                var bytesRead = stream.Read(tempBuffer, 0, (int)bufferLength);
                WriteBytes(tempBuffer.AsSpan(0, bytesRead));
                CheckMemoryLimit();
            }
        }
        ...
        private void FlushBlock()
        {
            ...
            BlockInfos.Add(new StorageBlockInfo((uint)_bufferStream.Length, (uint)compressedStream.Length,
                (StorageBlockFlags)CompressionType.Lzma));
