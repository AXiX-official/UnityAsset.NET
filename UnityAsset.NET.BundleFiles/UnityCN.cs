...
namespace UnityAsset.NET.BundleFiles
{
    public sealed unsafe class UnityCN
    {
        ...
        public UnityCN(string key)
        {
            ...
            InfoBytes = new byte[]
            {
                0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF, 0xA6, 0xB1, 0xDE, 0x48, 0x9E, 0x2B, 0x53, 0x5C
            };
            InfoKey = new byte[]
            {
            ...
            fixed (byte* targetPtr = &decompressedData.GetPinnableReference())
            {
                ...
                while (s < sourceEnd)
                {
                    ...
                    if (matchLength <= offset)
                    {
                        ...
                        {
                            #if NETSTANDARD2_1
                            *(Int64*)t = *(Int64*)(t - offset);
                            *(Int64*)(t + 8) = *(Int64*)(t - offset + 8);
                            #else
        ...
        private int EncryptByte(Span<byte> bytes, ref int offset, ref int index)
        {
            ...
            var b = Sub[((index >> 2) & 3) + 4] + Sub[index & 3] + Sub[((index >> 4) & 3) + 8] +
                    Sub[((byte)index >> 6) + 12];
